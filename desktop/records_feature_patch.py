...
if 'chRecordsWnd' not in s:
    s = replace_once(s, '''\tchAnalysisWnd uintptr
...
\tchBrowserPath string
''', 'chrome vars')
    # 1103 is intentionally outside the existing host command IDs (Signal Link uses 1003).
    s = replace_once(s, '\tchIDCArrow      = 32512\n)', '\tchIDCArrow      = 32512\n\tidRecords       = 1103 // '+MARK+'\n)', 'records command id')
    s = replace_once(s, '''\tif chWhatsappWnd != 0 {
...
''', 'records resize child')
    s = replace_once(s, '''\tif btnWhatsapp != 0 {
...
''', 'records button resize')
    s = replace_once(s, '''\tanalysis := chAnalysisWnd
...
''', 'desired view logic')
    s = replace_once(s, '''\t\tcase idWhatsapp:
\t\t\tchSwitchView(2)
\t\tcase chIDSignalLink:
''', '''\t\tcase idWhatsapp:
\t\t\tchSwitchView(2)
...
''', 'records command switch')
    s = replace_once(s, '''\tbtnWhatsapp, _, _ = chCreateWindowEx.Call(
...
''', 'records button create')
    s = replace_once(s, '''\t\tchSetWindowTheme.Call(btnAnalysis, uintptr(unsafe.Pointer(darkTheme)), 0)
...
''', 'records browser launch')
    s = replace_once(s, '''\tcmds := []*exec.Cmd{chAnalysisCmd, chWhatsappCmd}
