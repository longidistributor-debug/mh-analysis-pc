...
def replace_go_func(src: str, name: str, body: str) -> str:
    ...
    if start<0: raise SystemExit('missing Go function '+name)
    brace=src.find('{',start)
    if brace<0: raise SystemExit('missing brace for '+name)
    depth=0
    for i in range(brace,len(src)):
        if src[i]=='{': depth+=1
        elif src[i]=='}':
            depth-=1
            if depth==0: return src[:start]+body.rstrip()+src[i+1:]
...
if login_new not in s:
    if login_old in s: s=s.replace(login_old,login_new,1)
    else:
        m=list(re.finditer(r'(?m)^\s*hide\(j\);\s*$',s))
        if not m: raise SystemExit('successful login hide(j) anchor missing')
        q=m[-1]; indent=re.match(r'\s*',q.group(0)).group(0)
        s=s[:q.start()]+indent+'mhManualLoginV002 = true;\n'+indent+'hide(j); // '+MARK+s[q.end():]
s=re.sub(r'(?m)^\s*location\.reload\(\);\s*//?[^\n]*$', '', s)
s=re.sub(r'(?m)^\s*location\.reload\(\);\s*$', '', s)
if 'show("login_required")' not in s: raise SystemExit('Account Login UI missing')
...
resize=r'''func chResizeChildren() {
	...
	w := int32(r.R-r.L)
	if w < 1 { w = 1 }
	chViewMu.Lock(); locked:=chMandatoryUpdateV001; which:=chDesiredView; chViewMu.Unlock()
	if locked {
		fullH:=int32(r.B-r.T); if fullH<1 { fullH=1 }
		if chAnalysisWnd!=0 { chNormalizeEmbeddedTreeV001Final(chAnalysisWnd); chMoveWindow.Call(chAnalysisWnd,0,0,uintptr(w),uintptr(fullH),1); chShowWindow.Call(chAnalysisWnd,chSWShow) }
		if chWhatsappWnd!=0 { chShowWindow.Call(chWhatsappWnd,chSWHide) }
		if chRecordsWnd!=0 { chShowWindow.Call(chRecordsWnd,chSWHide) }
		if chMT5Wnd!=0 { chShowWindow.Call(chMT5Wnd,chSWHide) }
		for _,b:=range []uintptr{btnAnalysis,btnWhatsapp,btnRecords,btnMT5,chSignalLinkBtn} { if b!=0 { chShowWindow.Call(b,chSWHide) } }
		return
	}
	h := int32(r.B-r.T)-int32(barH); if h<1 { h=1 }
	layout:=func(child uintptr){ if child==0{return}; chNormalizeEmbeddedTreeV001Final(child); chMoveWindow.Call(child,0,uintptr(barH),uintptr(w),uintptr(h),1); chSetWindowPos.Call(child,0,0,uintptr(barH),uintptr(w),uintptr(h),chSWPNoZOrder|chSWPNoActivate|chSWPFrame) }
...
apply=r'''func chApplyDesiredBrowserView() {
	chMu.Lock(); analysis:=chAnalysisWnd; whatsapp:=chWhatsappWnd; records:=chRecordsWnd; mt5:=chMT5Wnd; chMu.Unlock()
	chViewMu.Lock(); which:=chDesiredView; locked:=chMandatoryUpdateV001; chViewMu.Unlock()
	chResizeChildren()
	if locked { chSetEmbeddedVisible(analysis,true); chSetEmbeddedVisible(whatsapp,false); chSetEmbeddedVisible(records,false); chSetEmbeddedVisible(mt5,false); return }
	...
	chResizeChildren()
	if which==3 { if analysis!=0 { chFocusEmbedded(analysis) } } else { chFocusDesiredEmbedded() }
}'''
s=replace_go_func(s,'chApplyDesiredBrowserView',apply)
switch=r'''func chSwitchView(which int) {
	chViewMu.Lock()
	if chMandatoryUpdateV001 { chViewMu.Unlock(); chApplyDesiredBrowserView(); return }
	if which<1 || which>4 { which=1 }; chDesiredView=which
	chViewMu.Unlock()
	...
	go func(){ for _,d:=range []time.Duration{30*time.Millisecond,120*time.Millisecond,320*time.Millisecond,700*time.Millisecond}{ time.Sleep(d); chMu.Lock(); stop:=chStopping; chMu.Unlock(); if stop{return}; chApplyDesiredBrowserView() } }()
}'''
s=replace_go_func(s,'chSwitchView',switch)
if 'func chLayoutUpdateChromeV001Final(' in s:
    s=replace_go_func(s,'chLayoutUpdateChromeV001Final',r'''func chLayoutUpdateChromeV001Final(hwnd uintptr, w, h int32) {
	if hwnd==0 { return }; chNormalizeEmbeddedTreeV001Final(hwnd); chMoveWindow.Call(hwnd,0,uintptr(barH),uintptr(w),uintptr(h),1); chSetWindowPos.Call(hwnd,0,0,uintptr(barH),uintptr(w),uintptr(h),chSWPNoZOrder|chSWPNoActivate|chSWPFrame)
}''')
p.write_text(s,encoding='utf-8')

p=Path('web/update-v001.css'); s=p.read_text(encoding='utf-8')
...
p.write_text(s,encoding='utf-8')
p=Path('updater.go'); s=p.read_text(encoding='utf-8'); s=re.sub(r'const mhPublicVersionV001 = "[^"]+"','const mhPublicVersionV001 = "V.02"',s); p.write_text(s,encoding='utf-8')
print(MARK+': fresh login + preserved data + working views + single frame + mandatory update lock preserved')
