...
function Click-Tab([IntPtr]$h,[int]$id,[string]$where){if(-not [MH814Win32]::PostMessage($h,0x0111,[IntPtr]$id,[IntPtr]::Zero)){throw "$where PostMessage failed"};Start-Sleep -Milliseconds 80}
function Wait-Different-Chrome([IntPtr]$h,[IntPtr]$old,[string]$where){for($i=0;$i -lt 220;$i++){[IntPtr]$v=Visible-Chrome $h;if($v -ne [IntPtr]::Zero -and $v -ne $old){Responsive $h "$where responsive";return $v};if(($i % 20)-eq 19){Responsive $h "$where waiting"};Start-Sleep -Milliseconds 150};throw "$where Records child never became visible"}
...
function Run-One([string]$label){$p=Start-Process $exe -PassThru;$server='';try{[IntPtr]$h=Main-Hwnd $p;$server=Wait-Server $p;Start-Sleep -Milliseconds 1600;Responsive $h "$label startup";[IntPtr]$analysis=Visible-Chrome $h;if($analysis -eq [IntPtr]::Zero){throw "$label analysis child missing"};Click-Tab $h 1005 "$label Records";[IntPtr]$records=Wait-Different-Chrome $h $analysis "$label Records";Assert-Geometry $h $records "$label Records";Responsive $h "$label records-first";
  ...
  Click-Tab $h 1001 "$label Analysis";for($i=0;$i -lt 50;$i++){[IntPtr]$analysis2=Visible-Chrome $h;if($analysis2 -ne [IntPtr]::Zero -and $analysis2 -ne $records){break};if($i -eq 49){throw "$label Analysis did not return"};Start-Sleep -Milliseconds 100};Responsive $h "$label analysis-return";Click-Tab $h 1005 "$label Records-return";for($i=0;$i -lt 50;$i++){[IntPtr]$v=Visible-Chrome $h;if($v -eq $records){break};if($i -eq 49){throw "$label Records reuse failed"};Start-Sleep -Milliseconds 100};Responsive $h "$label records-return";
