...
(() => {
  ...
  function textFor(code, fallback) {
    const map = {
      ...
      license_not_started: "Your license is not active yet.",
      ...
      database_not_configured: "License server is waiting for database configuration.",
  ...
  function show(code, msg) {
    ...
    meta.textContent = code === "license_expired" ? "EXPIRED • Contact administrator for renewal" : "";
  ...
  async function login(e) {
    ...
    if (!username || !password) { message.textContent = "Enter username and password."; return; }
    ...
    try {
      ...
      const j = await r.json().catch(() => ({}));
      if (!r.ok || !j.authorized) { show(j.code, j.message); return; }
      passInput.value = "";
      hide(j);
      location.reload();
    } catch { show("internet_required"); }
  ...
  window.fetch = async function(input, init) {
    ...
    try {
      ...
      if (r.status === 401 && String(u).includes("/api/") && !String(u).includes("/api/license/")) {
        const j = await r.clone().json().catch(() => ({}));
        show(j.code || "authorization_required", j.message);
      }
    } catch {}
    return r;
  };
  ...
  window.MHLicense = {
    logout: async () => { await rawFetch("/api/license/logout", { method: "POST" }).catch(() => {}); location.reload(); },
    status
  };
