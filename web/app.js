...
function analyze(c,prev){
  ...
  if(edge<=s.pressureUncertainty)return{signal:null,map:m,buyScore,sellScore,bestFamily:best,runnerUpFamily:runner,explanation:`No clear current directional edge: BUY ${buyScore} vs SELL ${sellScore}; difference ${two(edge)} is inside this timeframe's measured uncertainty ${two(s.pressureUncertainty)}.`,reasons,warnings};
...
function setContextMeta(count,source='API history'){
...
async function fetchCandles(targetSymbol=symbol,targetTimeframe=timeframe,{allowCacheFallback=true,reason='refresh'}={}){
  ...
  if(!accessKey)throw new Error('Save your API Access Key first.');
  ...
  const job=(async()=>{
    ...
    let r;
    try{
      r=await fetch('/api/history',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol:targetSymbol,period:targetTimeframe}),cache:'no-store'});
    }catch(e){
    ...
    if(!r.ok){
      const cached=candleCache.get(requestKey)||restoreCandles(requestKey)||[];
      if(allowCacheFallback&&cached.length>=60){
        if(requestKey===keyFor()){setDataState('warn',`API ${r.status} • cached ${cached.length}`);setContextMeta(cached.length,'Local cached history');setChartData(cached,false)}
        return{candles:cached.slice(-300),meta:{source:'cache fallback'},fromCache:true};
      }
      if(requestKey===keyFor())setDataState('bad',`Failed ${r.status}`);
      const extra=j.response_shape?` Response: ${j.response_shape}.`:'';
      throw new Error(`${j.error||`HTTP ${r.status}`}${extra}`);
    }
    ...
    if(requestKey===keyFor()){
      ...
      setContextMeta(out.length,'API history • Market chart');
...
async function loadContextChart(){
  ...
  if(!savedApiKey()){
    setDataState('neutralDot','API key not saved');
    $('#nativeFeedStatus').textContent=cached.length?`Cached ${cached.length} candles • save API key for fresh data`:'Save API Access Key to load chart';
    return cached;
  }
...
async function candlesForAnalysis(){
  ...
  return fetchCandles(symbol,timeframe,{allowCacheFallback:true,reason:'analysis'});
}
...
function renderTopMap(d){
  ...
  $('#mapFvg').textContent=m.fvgType?`${m.fvgType==='BULLISH'?'Bull':'Bear'} ${m.fvgFillPct}%`:'No';
  $('#mapEq').textContent=`${m.equalHigh!=null?'H':'-'} / ${m.equalLow!=null?'L':'-'}`;
...
function renderRecentSignals(){
  ...
  $('#recentSignals').innerHTML=previous.length?previous.map(x=>`<div class="recentRow"><span>${x.time}</span><b class="${x.label==='BUY'?'good':x.label==='SELL'?'bad':'warn'}">${x.label}</b><span>${x.score}/100</span></div>`).join(''):'No previous session signals yet.';
}
function addRecent(d,state='NEW'){
  const sig=d.signal,now=new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),label=sig?sig.direction:'NO EDGE',score=sig?.score??Math.max(d.buyScore||0,d.sellScore||0);
  const key=`${label}|${score}|${state}`;
  if(recentSession[0]?.key!==key)recentSession.unshift({time:now,label,score,state,key});
...
function renderDecision(d,reason,mode='NEW'){
  ...
  if(sig){
    ...
    b.className='signalBadge neutral';b.textContent='NO CLEAR EDGE';card.className='panel signalPanel neutralState';q.className='warn';q.textContent=`BUY ${d.buyScore} • SELL ${d.sellScore}`;plan.className='plan muted';plan.textContent='Fresh ranking found no statistically clear directional edge. The previous signal is not blindly reused.';
  }
...
function updateAutoButton(){
  ...
  const left=Math.max(0,autoSignalNextAt-Date.now()),m=Math.floor(left/60000),sec=Math.floor((left%60000)/1000);
  b.textContent=`Get Signal: ON • ${String(m).padStart(2,'0')}:${String(sec).padStart(2,'0')}`;
  if(autoSignalNextAt>0)setAutoStatus(`Next ${actionLabel(autoSignalNextAction)} in ${m}:${String(sec).padStart(2,'0')}`,'good');
}
...
function scheduleAutoAction(action,delayMs){
  ...
  autoSignalNextAction=action;autoSignalNextAt=Date.now()+Math.max(1000,delayMs);
  autoSignalTimer=setTimeout(()=>runScheduledAutoAction(action),Math.max(1000,delayMs));
...
async function autoSendAndSchedule(d,action,status=''){
  ...
  try{
    setAutoStatus(`Sending ${action.toLowerCase()}…`,'warn');
    await sendDecisionWhatsApp(d,action,status);
    if(action==='NEW ANALYSIS'){
      if(d?.signal){
        setAutoStatus(`Sent ${d.signal.direction} • RE-EVALUATE in 5m`,'good');
        scheduleAutoAction('REEVAL',AUTO_REEVAL_DELAY);
      }else{
        setAutoStatus('No signal sent • NEW ANALYZE again in 5m','warn');
        scheduleAutoAction('NEW',AUTO_RETRY_NO_SIGNAL_DELAY);
      }
    }else{
      setAutoStatus('Re-evaluation sent • NEW ANALYZE in 10m','good');
      scheduleAutoAction('NEW',AUTO_NEW_AFTER_REEVAL_DELAY);
    }
  }catch(e){
    setAutoStatus(e.message,'bad');
    // Keep the requested cycle alive even if WhatsApp temporarily fails.
    if(action==='NEW ANALYSIS')scheduleAutoAction(d?.signal?'REEVAL':'NEW',d?.signal?AUTO_REEVAL_DELAY:AUTO_RETRY_NO_SIGNAL_DELAY);
    else scheduleAutoAction('NEW',AUTO_NEW_AFTER_REEVAL_DELAY);
  }
}
async function setAutoSignalEnabled(on){
  if(on){
    await refreshBackendSettings();
    // Get Signal owns WhatsApp destination setup. KEYS owns only the market-data API key.
    ...
    if(!backendSettings.has_api_key){
      autoSignalEnabled=false;localStorage.setItem(STORAGE_AUTO,'0');stopAutoTimers();updateAutoButton();
      setAutoStatus('API key missing • add it from KEYS','bad');
      return;
    }
  }
  autoSignalEnabled=!!on;localStorage.setItem(STORAGE_AUTO,autoSignalEnabled?'1':'0');
  if(autoSignalEnabled){
    stopAutoTimers();autoSignalNextAt=Date.now();autoSignalNextAction='NEW';updateAutoButton();setAutoStatus('Starting NEW ANALYZE now…','warn');
    setTimeout(()=>runScheduledAutoAction('NEW'),100);
...
async function executeNewAnalysis(fromAuto=false){
  if(busy)return null;busy=true;setBusy(true,`${fromAuto?'AUTO • ':''}NEW ANALYZE • fetching one fresh candle snapshot…`);
  try{
    const f=await candlesForAnalysis(),c=f.candles,k=keyFor(),prev=(active.get(k)||restoreActiveSignal(k))?.signal||null,d=analyze(c,prev);d._ranked=buildRankedForUi(c,d);const reason=refreshReason(prev,d);
    ...
    showError(e.message);if(autoSignalEnabled){setAutoStatus(`Analysis failed • retry in 5m`,'bad');scheduleAutoAction('NEW',AUTO_RETRY_NO_SIGNAL_DELAY)}return null;
  }finally{busy=false;setBusy(false)}
}
async function executeReevaluate(fromAuto=false){
  if(busy)return null;const k=keyFor(),a=active.get(k)||restoreActiveSignal(k);
  if(!a){if(!fromAuto)showError(`No active ${symbol} ${timeframe} signal. Press NEW ANALYZE first.`);if(autoSignalEnabled){setAutoStatus('No active signal • NEW ANALYZE in 5m','warn');scheduleAutoAction('NEW',AUTO_RETRY_NO_SIGNAL_DELAY)}return null}
  busy=true;setBusy(true,`${fromAuto?'AUTO • ':''}RE-EVALUATE • testing the original signal against fresh data…`);
  try{
    const f=await candlesForAnalysis(),c=f.candles,s=a.signal,current=analyze(c,s),age=signalBarsAge(c,s),life=adaptiveExpiryBars(c),last=c.at(-1),model=empiricalDistanceModel(c,s.direction,stats(c));
    ...
    if(slHit){status='INVALID — original structural invalidation / SL was breached';}
    ...
    if(autoSignalEnabled)await autoSendAndSchedule(current,'RE-EVALUATE',status);return current;
  }catch(e){showError(e.message);if(autoSignalEnabled){setAutoStatus('Re-evaluate failed • NEW ANALYZE in 10m','bad');scheduleAutoAction('NEW',AUTO_NEW_AFTER_REEVAL_DELAY)}return null}
  finally{busy=false;setBusy(false)}
}
...
function setupLightweightChart(){
  ...
  volumeSeries=lwChart.addSeries(LightweightCharts.HistogramSeries,{priceScaleId:'vol',priceFormat:{type:'volume'},lastValueVisible:false,priceLineVisible:false});
  try{volumeSeries.priceScale().applyOptions({scaleMargins:{top:.82,bottom:0}})}catch(e){}
...
function renderLoadedMap(arr){try{if(!arr?.length)return;const m=marketMap(arr),reg=m.adx>=28?'TRENDING':m.adx<=16?'RANGING':'MIXED';$('#mapRegime').textContent=reg;$('#mapRegime').className=reg==='TRENDING'?'good':reg==='RANGING'?'warn':'';$('#mapAdx').textContent=Number.isFinite(m.adx)?m.adx.toFixed(1):'—';$('#mapVwap').textContent=fmt(m.vwap);$('#mapBullOb').textContent=m.bullObLow!=null?'Yes':'No';$('#mapBullOb').className=m.bullObLow!=null?'good':'';$('#mapBearOb').textContent=m.bearObLow!=null?'Yes':'No';$('#mapBearOb').className=m.bearObLow!=null?'bad':'';$('#mapFvg').textContent=m.fvgType?`${m.fvgType==='BULLISH'?'Bull':'Bear'} ${m.fvgFillPct}%`:'No';$('#mapEq').textContent=`${m.equalHigh!=null?'H':'-'} / ${m.equalLow!=null?'L':'-'}`;$('#mapDiv').textContent=m.divergence||'No'}catch(e){}}
function setChartData(arr,fit=false){
  ...
  volumeSeries?.setData(arr.map(x=>({time:Math.floor(x.t),value:Number(x.v||0),color:x.c>=x.o?'rgba(0,199,163,.42)':'rgba(255,69,93,.42)'})));
  renderLoadedMap(arr);
  renderSignalLevels();
...
function renderSignalLevels(){
  ...
  const d=lastDecision.get(keyFor())||chartDecision,sig=d?.signal;if(!sig)return;
  const isBuy=sig.direction==='BUY';
...
function drawRsiChart(canvas,arr){if(!canvas)return;const {w,h,d}=canvasSize(canvas),ctx=canvas.getContext('2d');ctx.clearRect(0,0,w,h);ctx.fillStyle='#050a0d';ctx.fillRect(0,0,w,h);if(!arr.length)return;const closes=arr.map(x=>x.c),vals=[];for(let i=0;i<closes.length;i++)vals.push(i<14?50:rsi(closes.slice(0,i+1),14));const y=v=>6*d+(100-v)/100*(h-12*d),x=i=>8*d+i*(w-16*d)/Math.max(1,vals.length-1);[30,50,70].forEach(v=>{drawLine(ctx,8*d,y(v),w-8*d,y(v),v===50?'#263b36':'#51435e',1*d,[3*d,4*d]);ctx.fillStyle='#82948f';ctx.font=`${8*d}px Segoe UI`;ctx.fillText(String(v),w-25*d,y(v)-4*d)});ctx.strokeStyle='#8b6bd6';ctx.lineWidth=1.2*d;ctx.beginPath();vals.forEach((v,i)=>{const xx=x(i),yy=y(v);if(i===0)ctx.moveTo(xx,yy);else ctx.lineTo(xx,yy)});ctx.stroke();ctx.fillStyle='#b6c5c1';ctx.fillText(`RSI ${vals.at(-1).toFixed(1)}`,8*d,13*d)}
function loadChart(){
  $('#chartLabel').textContent=`${symbol} • ${timeframe}`;$('#currentPair').textContent=`${symbol} • ${timeframe}`;$('#nativeSymbol').textContent=symbol;
  $$('.nativeTfButtons button').forEach(b=>b.classList.toggle('active',b.dataset.chartTf===timeframe));
  setChartData(candleCache.get(keyFor())||restoreCandles(keyFor())||[],true);
}
...
async function contextChanged(){
  loadChart();resetViewForContext();refreshMarketCap();$('#nativeFeedStatus').textContent='Loading latest candles…';await loadContextChart();refreshMarketCap();
}
...
async function openNativeApiSettings(){try{const v=window.prompt('API Access Key\n\nPaste/type a new key. Leave blank and press OK to clear it.','');if(v===null)return;await saveBackendSetting({api_key:String(v).trim()});setDataState(backendSettings.has_api_key?'neutralDot':'warn',backendSettings.has_api_key?'Key saved':'API key not saved')}catch(e){showError('Could not save API Key: '+(e.message||e))}}
async function openWhatsappSettingsPrompt(){try{await refreshBackendSettings();const current=backendSettings.whatsapp_link||'';const v=window.prompt('WhatsApp Signal Link / Number\n\nExample: https://wa.me/923434824609',current);if(v===null)return false;await saveBackendSetting({whatsapp_link:String(v).trim()});return !!backendSettings.has_whatsapp}catch(e){setAutoStatus('Could not save WhatsApp link','bad');return false}}
...
await refreshBackendSettings();
if(backendSettings.has_api_key){setDataState('neutralDot','Key saved')}else setDataState('neutralDot','API key not saved');
...
if(autoSignalEnabled)setTimeout(()=>setAutoSignalEnabled(true),700);else updateAutoButton();
})();