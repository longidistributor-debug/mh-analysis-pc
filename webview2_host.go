...
func wv2ShowLocal(which int) {
	...
	_ = wv2Browser.Show()
	wv2Resize()
	if which == 2 { wv2Browser.Navigate("https://web.whatsapp.com/") } else if which == 3 { wv2Browser.Navigate(serverURL+"records.html") } else { wv2Browser.Navigate(serverURL) }
	wv2Browser.Focus()
}

func wv2ButtonAllowed(id int) bool {
	if licAuthorized { return true }
	section := "MH Analysis"; if id==idWhatsapp {section="WhatsApp"}; if id==idRecords {section="Records"}; if id==idMT5 {section="MT5 System"}
...
func wv2WndProc(hwnd uintptr, msg uint32, wp, lp uintptr) uintptr {
	...
	case chWMSize:
		wv2Resize(); return 0
	case chWMCommand:
		id:=int(wp&0xffff); if !wv2ButtonAllowed(id){return 0}
		switch id { case idAnalysis: wv2ShowLocal(1); case idWhatsapp: wv2ShowLocal(2); case idRecords: wv2ShowLocal(3); case idMT5: _=wv2Browser.Hide(); chShowWindow.Call(wv2Container,chSWHide); go func(){if err:=chEnsureMT5Terminal();err!=nil{messageBox(hostHWND,err.Error(),"MT5 System",0x10)}}() }
		return 0
	...
	case chWMClose: chShowWindow.Call(hwnd,chSWHide); if wv2Browser!=nil{_=wv2Browser.Hide()}; if chMT5Wnd!=0{chShowWindow.Call(chMT5Wnd,chSWHide)}; chDestroyWindow.Call(hwnd); return 0
...
func runWebView2Host() {
	...
	wv2Resize(); b.Navigate(serverURL); b.Focus()
