...
func chSettingsWndProc(hwnd uintptr, msg uint32, wParam, lParam uintptr) uintptr {
	...
	case chWMCommand:
		...
		case chIDSettingsSave:
			...
			if edit == 0 {
				messageBox(hwnd, "Could not read setting field.", "MH Analysis", 0x10)
				return 0
			}
...
func chShowNativeSettingsDialog(mode int) {
	...
	if mode == 2 {
		...
		label = "WhatsApp Signal Link / Number"
		value = v.WhatsAppLink
	}

	const dlgW int32 = 680
	const dlgH int32 = 235
	...
	chCreateWindowEx.Call(0, uintptr(unsafe.Pointer(chWstr("BUTTON"))), uintptr(unsafe.Pointer(chWstr("SAVE"))), chWSChild|chWSVisible|chWSTabStop, 20, 105, 150, 36, chSettingsDialog, chIDSettingsSave, inst, 0)
	chCreateWindowEx.Call(0, uintptr(unsafe.Pointer(chWstr("BUTTON"))), uintptr(unsafe.Pointer(chWstr("CLEAR"))), chWSChild|chWSVisible|chWSTabStop, 180, 105, 150, 36, chSettingsDialog, chIDSettingsClear, inst, 0)
	chCreateWindowEx.Call(0, uintptr(unsafe.Pointer(chWstr("BUTTON"))), uintptr(unsafe.Pointer(chWstr("CANCEL"))), chWSChild|chWSVisible|chWSTabStop, 340, 105, 150, 36, chSettingsDialog, chIDSettingsCancel, inst, 0)
