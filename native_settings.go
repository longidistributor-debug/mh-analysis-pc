...
func chSaveNativeSettings(mode int, value1, value2 string) error {
	return chSaveNativeSetting(mode, value1)
}

func chSettingsEditFor(hwnd uintptr) uintptr {
	if hwnd != 0 {
		if h, _, _ := chSettingsGetDlgItem.Call(hwnd, chIDSettingsEdit); h != 0 { return h }
	}
...
func chSettingsWndProc(hwnd uintptr, msg uint32, wParam, lParam uintptr) uintptr {
	...
	case chWMCommand:
		...
		case chIDSettingsSave:
			...
			if err := chSaveNativeSetting(chSettingsMode, chWindowText(edit)); err != nil {
				messageBox(hwnd, "Could not save setting: "+err.Error(), "MH Analysis", 0x10); return 0
			}
			chDestroyWindow.Call(hwnd); return 0
		case chIDSettingsClear:
			if err := chSaveNativeSetting(chSettingsMode, ""); err != nil {
				messageBox(hwnd, "Could not clear setting: "+err.Error(), "MH Analysis", 0x10); return 0
			}
			chDestroyWindow.Call(hwnd); return 0
	...
	case chWMDestroy:
		if hwnd == chSettingsDialog {
			chSettingsDialog = 0; chSettingsEdit = 0; chSettingsMode = 0
		}
		return 0
...
func chEnsureSettingsClass() bool {
	if chSettingsClassOK { return true }
	inst, _, _ := chGetModuleHandle.Call(0)
	cursor, _, _ := chLoadCursor.Call(0, chIDCArrow)
	if chSettingsBrush == 0 { chSettingsBrush, _, _ = chCreateSolidBrush.Call(uintptr(8 | (22 << 8) | (26 << 16))) }
	wc := chWndClassEx{CbSize: uint32(unsafe.Sizeof(chWndClassEx{})), LpfnWndProc: syscall.NewCallback(chSettingsWndProc), HInstance: inst, HCursor: cursor, HbrBackground: chSettingsBrush, LpszClassName: chWstr("MHAnalysisSettingsDialog")}
...
func chShowNativeSettingsDialog(mode int) {
	...
	if chSettingsDialog != 0 {
		if chSettingsMode == mode {
			...
			if edit := chSettingsEditFor(chSettingsDialog); edit != 0 {
				chSettingsSetFocus.Call(edit); chSettingsSendMessage.Call(edit, chEMSetSel, 0, ^uintptr(0))
			}
			return
		}
		chDestroyWindow.Call(chSettingsDialog)
	}
	if !chEnsureSettingsClass() { return }
	...
	value := v.APIKey
	if mode == 2 {
		title = "WhatsApp Signal Link"
		label = "WhatsApp Signal Link / Group Link"
	...
	if hostHWND != 0 {
		...
		if ok, _, _ := chSettingsGetWindowRect.Call(hostHWND, uintptr(unsafe.Pointer(&rr))); ok != 0 {
			x = rr.L + (rr.R-rr.L-dlgW)/2; y = rr.T + (rr.B-rr.T-dlgH)/2
	...
	inst, _, _ := chGetModuleHandle.Call(0)
	chSettingsMode = mode
	chSettingsDialog, _, _ = chCreateWindowEx.Call(0, uintptr(unsafe.Pointer(chWstr("MHAnalysisSettingsDialog"))), uintptr(unsafe.Pointer(chWstr(title))), chWSOverlapped|chWSVisible, uintptr(x), uintptr(y), uintptr(dlgW), uintptr(dlgH), hostHWND, 0, inst, 0)
	if chSettingsDialog == 0 { chSettingsMode = 0; return }
	...
	dark := int32(1)
	if chDwmSetWindowAttribute.Find() == nil { chDwmSetWindowAttribute.Call(chSettingsDialog, 20, uintptr(unsafe.Pointer(&dark)), unsafe.Sizeof(dark)) }
	chShowWindow.Call(chSettingsDialog, chSWShow)
	chUpdateWindow.Call(chSettingsDialog)
	chSetForegroundWindow.Call(chSettingsDialog)
	if chSettingsEdit != 0 { chSettingsSetFocus.Call(chSettingsEdit); chSettingsSendMessage.Call(chSettingsEdit, chEMSetSel, 0, ^uintptr(0)) }
}
