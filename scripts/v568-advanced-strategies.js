...
function v568ImpulseBaseFamilies(c,m,s){
  const tr=Math.max(s.trMedian,1e-9),w=c.slice(-14),out=[]; if(w.length<10)return out;
...
function v568InducementFamilies(c,m,s){
  const q=v568LiquidityState(c,m,s),last=c.at(-1),tr=Math.max(s.trMedian,1e-9),tol=tr*.22,bs=v568RecentSweep(c,'LOW',s,7),ss=v568RecentSweep(c,'HIGH',s,7);
