...
// V.55.3 isolated control/scalp mailboxes. Normal signal.txt/manage.txt behavior is untouched.
func eaWriteIsolatedMailbox(name, line string) error {
...
func eaSignalSendHandler(w http.ResponseWriter, r *http.Request) {
	...
	// V.55.3 configuration is isolated from normal signal.txt so existing EA behavior cannot collide.
	if q.Type=="MH_CONFIG" {
		if q.FastLot<=0 || q.FastLot>0.01 { q.FastLot=0.01 }
		if q.FastTPMin<=0 { q.FastTPMin=2 }
		if q.FastTPMax<q.FastTPMin { q.FastTPMax=q.FastTPMin }
		if q.DailyLossLimit<=0 { q.DailyLossLimit=100 }
		if q.DailyProfitStop<=0 { q.DailyProfitStop=200 }
		line:=strings.Join([]string{"V.55.3",strconv.FormatBool(q.SLAdjustment),strconv.FormatBool(q.ProgressiveV1),strconv.FormatBool(q.FastScalping),strconv.FormatFloat(q.FastLot,'f',2,64),strconv.FormatFloat(q.FastTPMin,'f',2,64),strconv.FormatFloat(q.FastTPMax,'f',2,64),strconv.FormatFloat(q.DailyLossLimit,'f',2,64),strconv.FormatFloat(q.DailyProfitStop,'f',2,64)},"|")+"\r\n"
		if err:=eaWriteIsolatedMailbox("config.txt",line);err!=nil{w.WriteHeader(http.StatusInternalServerError);_=json.NewEncoder(w).Encode(map[string]any{"error":"could not publish EA config: "+err.Error()});return}
		_=json.NewEncoder(w).Encode(map[string]any{"ok":true,"mode":"config"});return
	}
	if q.Type=="FAST_BUY" || q.Type=="FAST_SELL" {
		...
		if err:=eaWriteIsolatedMailbox("fast_scalp.txt",line);err!=nil{w.WriteHeader(http.StatusInternalServerError);_=json.NewEncoder(w).Encode(map[string]any{"error":"could not publish fast scalp: "+err.Error()});return}
		_=json.NewEncoder(w).Encode(map[string]any{"ok":true,"signal_id":q.SignalID,"symbol":q.Symbol,"type":q.Type,"lot":0.01});return
	}
