...
export default async function handler(req,res){
 ...
 try{
  ...
  if(!user||!await bcrypt.compare(pw,user.passwordHash)){await log(d,u,'LOGIN_FAILED',ip,deviceId,{reason:'credentials'});return out(res,401,{ok:false,error:'INVALID_CREDENTIALS'});}
  ...
  const ch=await d.collection('challenges').findOne({_id:cid});if(!ch||ch.used||ch.expiresAt<=new Date()||ch.usernameHash!==hash(u)||ch.deviceIdHash!==hash(deviceId)){await log(d,u,'LOGIN_FAILED',ip,deviceId,{reason:'challenge'});return out(res,401,{ok:false,error:'INVALID_CHALLENGE'});}
