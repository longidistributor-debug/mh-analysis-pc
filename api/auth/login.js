...
export default async function handler(req,res){
 ...
 try{
  ...
  if(user.device?.publicKeyFingerprint&&user.device.publicKeyFingerprint!==fingerprint){await log(d,u,'UNAUTHORIZED_DEVICE',ip,deviceId);return out(res,403,{ok:false,error:'DEVICE_NOT_AUTHORIZED',message:'This account is already activated on another device. Contact administrator.'});}
  const now=new Date(),first=!user.device?.publicKeyFingerprint,clean={device_name:String(info.device_name||info.machine_name||'').slice(0,120),os_version:String(info.os_version||info.os||'').slice(0,120),arch:String(info.arch||'').slice(0,40),app_version:String(info.app_version||'').slice(0,40)},device=first?{deviceId,publicKeyFingerprint:fingerprint,publicKey:pub,info:clean,boundAt:now,lastSeenAt:now,firstIpHash:hash(ip),lastIpHash:hash(ip)}:{...user.device,deviceId,info:{...user.device.info,...clean},lastSeenAt:now,lastIpHash:hash(ip)};
  await users.updateOne({_id:user._id},{$set:{device,lastLoginAt:now,updatedAt:now}});await log(d,u,first?'DEVICE_BOUND':'LOGIN_SUCCESS',ip,deviceId);console.info('AUTH_SUCCESS',{username:u,firstDevice:first});const tv=user.tokenVersion||1;return out(res,200,{ok:true,access_token:signToken({sub:String(user._id),username:u,dfp:fingerprint,tv},1800,'mh-analysis'),expires_in:1800,username:u,valid_from:user.validFrom,valid_until:user.validUntil,device_bound:true,max_devices:1});
