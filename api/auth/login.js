...
export default async function handler(req,res){
 ...
 try{
  ...
  // Secure legacy migration: an already-bound installation may adopt the corrected stable MachineGuid only when it proves possession of the SAME existing private key. A different PC cannot satisfy this signature/fingerprint check.
  const sameBoundKey=Boolean(storedFingerprint&&storedFingerprint===fingerprint);
  if(storedMachineIdHash&&storedMachineIdHash!==machineIdHash&&!sameBoundKey){await log(d,u,'UNAUTHORIZED_DEVICE',ip,deviceId,{reason:'machine_mismatch'});return out(res,403,{ok:false,error:'DEVICE_NOT_AUTHORIZED',message:'This account is already activated on another device. Contact administrator.'});}
  if(!storedMachineIdHash&&storedFingerprint&&storedFingerprint!==fingerprint){await log(d,u,'UNAUTHORIZED_DEVICE',ip,deviceId,{reason:'legacy_key_mismatch'});return out(res,403,{ok:false,error:'DEVICE_NOT_AUTHORIZED',message:'This account is already activated on another device. Contact administrator.'});}
